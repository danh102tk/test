...
async def upload_document(file: UploadFile=File(...)):
    ...
    temp_path=settings.upload_dir/f'{safe_name}'
    with temp_path.open('wb') as f: shutil.copyfileobj(file.file,f)
    try:
    ...
    return {'document_id':result.document_id,'status':'processed','summary':{'pages':result.page_count,'groups':len(result.groups),'employees':len(result.employees),'issues':len(result.issues),'confidence':result.overall_confidence,'engine':result.processing_engine}}
