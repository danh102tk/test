...
Priority: PaddleOCR (light models) -> Tesseract fallback.
Avoids PP-OCRv6 medium (often hangs on Windows).
"""
...
# Disable oneDNN / PIR before importing paddle
os.environ.setdefault("FLAGS_use_mkldnn", "0")
os.environ.setdefault("FLAGS_onednn", "0")
os.environ.setdefault("FLAGS_use_onednn", "0")
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "")  # force CPU if needed
...
def _get_paddle(lang: str = "en"):
    ...
    # Lighter models; less likely to hang than PP-OCRv6 medium
    candidates = [
        # Prefer mobile / v4 models when API supports them
        dict(
            lang=lang,
            device="cpu",
...
def extract_ocr(pdf_path, lang: str = "en", use_gpu: bool = False):
    """Alias for engines API."""
    from pathlib import Path as P
    r = process_pdf(P(pdf_path), lang=lang, use_gpu=use_gpu)
    if not r:
        return None
    from app.engines.base import TextEngineResult
    return TextEngineResult(engine=r["engine"], pages=r["pages"])
